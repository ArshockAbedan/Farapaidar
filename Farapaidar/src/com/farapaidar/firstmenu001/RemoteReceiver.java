...
public class RemoteReceiver extends DialogFragment {
	...
	static RemoteReceiver newInstance() {
		return new RemoteReceiver();
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View v = inflater.inflate(R.layout.activity_remote_receiver, container,
				false);

		// Set onClickEvent for BtnContacts
		Button btnUpCt = (Button) v.findViewById(R.id.btnContacts);
		...
		Button btnUpGd = (Button) v.findViewById(R.id.btnGoods);
		...
		Button btnUpPricelist = (Button) v.findViewById(R.id.btnPriceList);
	...
	public void UpdateContacts(View v, Context contaxt) {
		...
		try {
			...
			resultSetContacts = stmtContacts
					.executeQuery(" SELECT * FROM vw_Apk_Cust");
			...
			while (resultSetContacts.next()) {
				...
				try {
					if (resultSetContacts.getString(15).trim().length() != 0) {
						guidCustomerCategory = resultSetContacts.getString(15)
								.toString();
					}
				} catch (Exception e) {
					// TODO: handle exception
	...
	public void readBultainFromServer() {
		...
		try {
			...
			stmtContacts = DbConnContacts.createStatement();
			resultSetContacts = stmtContacts
					.executeQuery(" select TXT from Bultain Order by id DESC");

			ArrayList<Bultain> bultains = new ArrayList<Bultain>();

			while (resultSetContacts.next()) {
				Bultain blt = new Bultain();

				String txt = "-";
				try {
					if (resultSetContacts.getString(1).trim().length() != 0) {
						txt = resultSetContacts.getString(1).toString();
					}
				} catch (Exception e) {
					// TODO: handle exception
				}
			...
			// First delete existing records in Bultain table
			db.deleteAllBultain();

			for (Bultain bultain : bultains) {
				db.addBultain(bultain);
			}

		} catch (Exception e) {
			// TODO: handle exception
			Toast.makeText(this.getActivity().getBaseContext(), e.getMessage(),
					Toast.LENGTH_SHORT).show();
		}
	}
