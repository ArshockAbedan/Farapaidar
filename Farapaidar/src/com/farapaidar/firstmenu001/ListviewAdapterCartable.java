...
public class ListviewAdapterCartable extends BaseAdapter{

	@SuppressWarnings("rawtypes")
	public ArrayList<HashMap> list;
    Activity activity;
    
    public ListviewAdapterCartable(Activity activity, @SuppressWarnings("rawtypes") ArrayList<HashMap> list) {
        super();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		// TODO Auto-generated method stub
         ...
         if (convertView == null)
         {
             convertView = inflater.inflate(R.layout.cartablelistview_row, null);
		...
		HashMap map = list.get(position);
			...
			{         
				...
				holder.txtFirst.setTypeface(tf, Typeface.BOLD);
			}
			...
			{    
				holder.txtSecond.setTag(map.get(FIFTH_COLUMN).toString());
				if (map.get(FIFTH_COLUMN).toString().equalsIgnoreCase("0"))
				{
					holder.txtFirst.setTypeface(tf, Typeface.NORMAL);
					holder.txtSecond.setTypeface(tf, Typeface.NORMAL);
					holder.txtThird.setTypeface(tf, Typeface.NORMAL);
					
				}else if (map.get(FIFTH_COLUMN).toString().equalsIgnoreCase("1"))
				{
					holder.txtFirst.setTypeface(tf, Typeface.BOLD_ITALIC);
					holder.txtSecond.setTypeface(tf, Typeface.BOLD_ITALIC);
					holder.txtThird.setTypeface(tf, Typeface.BOLD_ITALIC);
				}
			}
